...
# read in list of userid replacements
...
survey <- read_csv(here("data", "survey", "Survey - GenAI_March 13, 2024_15.24.csv")) %>% # this one has more rows than one from Feb, so I assume it's the right one
  ...
  relocate(
    c(id_1:id_5), .after = user_language
  ) %>% 
  mutate(
...
write_csv(exp_merged, here("data", "exp_merged.csv"))
write_rds(exp_merged, here("data", "exp_merged.rds"))

exp_merged %>% 
  filter(userid == "ls16la")
```
...
union(names(exp_merged), names(survey)) %>%
                      length()  # 157 unique variables

# create list of matching userids
exp_merged_mathed <- exp_merged %>% 
  mutate(userid = ifelse(userid == id_replacements$experiment, id_replacements$survey, userid))

exp_merged_mathed %>% filter(userid == "ae04hf")

exp_merged %>% filter(userid == "as04hf")

exp_merged$userid == exp_merged_mathed$userid

# left join the survey with experiment data
df_merged_matched <- df_merged <- exp_merged %>% 
  left_join(survey)

# write to RDS and CSV
write_csv(exp_merged, here("data", "df_merged.csv"))
write_rds(exp_merged, here("data", "df_merged.rds"))
```
- final dataset has 218 obs of 157 vars

```{r}
id_replacements <- tibble(
  survey = c("ad10sd", "ae04hf", "bl14ja", "cg19lo", "ch04ja", 
             "js03hj", "js06zm", "ks05hn", "lc22bj", "rs12ys", 
             "sc24hs", "sc02lj", "sx06yd", "ts14qj", "wz04qj", 
             "xl24lj", "yh19yf", "ys27hj", "ys09hj", "yw11lj", 
             "yz12sj", "yz14yo", "sx19sm", "cd09re", "zw20jj", 
             "yj28sa", "kd27nj", "jl10mn", "js16la"),
  experiment = c("ad10sd", "as04hf", "bl14ha", "wg19lo", "ch04jm", 
                 "jh03hj", "js06jm", "kt05hn", "lz22bj", "rs12ws", 
                 "sc24ho", "sc02hj", "sx06md", "tt14qj", "wj04qj", 
                 "xa24lj", "th19yf", "ys27lj", "ys09xj", "jw11lj", 
                 "yz06sj", "yz14to", "sx19sm", "cd09rn", "zw20jd", 
                 "yj28za", "kn27nj", "jp10mn", "ls16la")
)

write_csv(id_replacements, here("data", "id_replacements.csv"))
```
