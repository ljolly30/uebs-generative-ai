...
ug_survey <- read_csv(here("data", "survey", "Survey - GenAI_March 13, 2024_15.24.csv")) %>% # this one has more rows than one from Feb, so I assume it's the right one
  ...
  filter(progress >= 87 & consent == "yes") %>% # filter for completion rate > 87% (Jordana's cutoff) and consenting ppts
...
ug_survey %>% 
  count(userid) %>% 
  filter(n>1) %>% 
  summarise(sum(n)) # 24 duplicate entries
```
- 228 unique userids
...
ug_exp %>% 
  count(userid) %>% 
  filter(n>1) %>% 
  summarise(sum(n)) # 59 duplicate entries (32 of which are)
```
- 198 unique userids
...
pg_exp <- read_csv(here("data", "experiment", "MSc Experiment March 13, 2024_15.25.csv"), col_names = T) %>%
  ...
  filter(progress >= 87) # filter for completion rate > 87% (Jordana's cutoff) - everyone is above this
...
pg_exp %>% 
  count(userid) %>% 
  filter(n>1) %>% 
  summarise(sum(n)) # no duplicate entries
```
